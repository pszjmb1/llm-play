...
export async function getSecureUploadUrl(
  ...
  if (!isValidFileType(fileType)) {
    throw new SubmissionError(`Unsupported file type: ${fileType}`, 'FILE_UPLOAD', {
      allowedTypes: ALLOWED_FILE_TYPES,
    });
  }

  if (!isValidExtension(`.${fileExtension}`)) {
    throw new SubmissionError(`Unsupported file extension: ${fileExtension}`, 'FILE_UPLOAD', {
  ...
  try {
    ...
    if (error) {
      ...
      throw new SubmissionError('Failed to prepare file upload', 'FILE_UPLOAD', error);
    }

    if (!data?.signedUrl) {
      throw new SubmissionError('No signed URL returned from server', 'FILE_UPLOAD');
    }
    ...
    throw new SubmissionError('Unexpected error preparing file upload', 'UNKNOWN', error);
  }
}
...
export async function getPublicFileUrl(filePath: string): Promise<string> {
  ...
  const { data } = supabase.storage.from('environments').getPublicUrl(filePath);

  if (!data.publicUrl) {
    throw new SubmissionError('Failed to generate public URL for file', 'FILE_UPLOAD');
  }
...
export async function verifyFileExists(filePath: string): Promise<boolean> {
  ...
  try {
    ...
    const { data, error } = await supabase.storage.from('environments').download(filePath);
...
export async function deleteStorageFile(filePath: string): Promise<boolean> {
  ...
  try {
    const { error } = await supabase.storage.from('environments').remove([filePath]);
