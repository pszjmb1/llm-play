...
export async function uploadFile(file: File, folderPath = 'environments'): Promise<string> {
  ...
  const { data, error } = await supabase.storage.from('environments').upload(filePath, file, {
    cacheControl: '3600',
    upsert: false,
  });
  ...
  // Get the public URL
  const { data: publicUrlData } = supabase.storage.from('environments').getPublicUrl(data.path);
